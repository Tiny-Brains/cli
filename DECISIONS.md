...
| Plain series: the training environment (47, 48 of the loader's) and the run's own clock (60) | [cli](https://github.com/Tiny-Brains/cli/blob/main/DECISIONS.md) |
...
| **N3, N6–N8, N12, N13, N15–N19** · **N1, N2, N4, N5, N9** · **N20–N22, N24, N27** · **N23** · **N25** | soma · kalam · ants · cli · web |
...
### The binary's own measurements

| # | Decision | Taken as |
|---|---|---|
| **60** | **A run measures itself, and the breakdown is printed against the clock it came from** | `src/timing.rs` accumulates nanoseconds and call counts per phase, written from the cartridge host, datalogic, tract and the wave loop, and `--timings` prints them. Two properties make it honest rather than decorative. **The phases are declared disjoint**, `Registry`..`Write`, and what they do not account for is printed as its own `unaccounted` row against the wall clock the command measured — a breakdown that normalises to 100% of itself cannot tell you it is missing something, and this one can. **The cartridge calls are decomposed separately** (instantiate, encode, the wasm call, decode) and double-count on purpose, because the question "is it the sandbox or the payload?" is not answerable from a disjoint list. Recording is unconditional — two atomics and an `Instant` per span, tens of nanoseconds against the milliseconds being measured — so there is no build flag, no feature and no second code path to drift; only the printing is behind the flag, and a run without it prints exactly what it printed before. The graph node by node (`TINYBRAINS_PROFILE_NODES`) is the one exception: it swaps `plan.run` for tract's state machine, so it is opt-in and says so. **Why the binary and not a profiler:** `sample` and Instruments answer "which symbol", and the question a competitor has is "which *phase*" — the adapter they wrote against the graph they trained, the sandbox against the model. A symbol profile cannot separate `observe` from `step`, because both are one `call_invoke`. What it found on ants-starter's own match is in README's Status |
